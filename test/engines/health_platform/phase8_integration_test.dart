...
void main() {
  ...
  test('HealthPlatformRule is observe-only and priorities include it', () {
    expect(
      DecisionPriorities.domainEvaluationOrder,
      contains('HealthPlatformRule'),
    );

    final snap = HealthPlatformSnapshot(
      userId: 'user-1',
      computedAt: now,
      hydrationPercent: 40,
      hasBpCaution: true,
      overallCaution: HealthCautionLevel.caution,
    );
    ...
    final plan = _engine().buildDailyPlan(
      ...
      buildTestProfile(),
      now: now,
      healthPlatformSnapshot: snap,
    );

    expect(plan.hasHealthPlatform, isTrue);
    expect(plan.finalDecision.adjustment, WorkoutAdjustment.none);
    expect(plan.finalDecision.requiresAdaptation, isFalse);

    final coach = const CoachContextBuilder().build(plan);
    expect(coach.hasBpCaution, isTrue);
    expect(coach.hydrationPercent, 40);
    final map = coach.toPromptMap();
    expect(map.containsKey('systolic'), isFalse);
    expect(map['hasBpCaution'], isTrue);
  });
}
